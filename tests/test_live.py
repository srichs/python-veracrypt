...
if __name__ == '__main__':
    try:
        vc = VeraCrypt(logging.DEBUG, log_fmt="%(asctime)s:%(levelname)s:%(module)s:%(funcName)s:%(message)s")
        username = 'user'
        volume_path = ''
        ...
        if vc.os_name == 'Windows':
            volume_path = os.path.join('C:\\', 'Users', username, 'Desktop', volume_name)
            ...
            if vc.os_name == 'Darwin':  # macOS
                volume_path = os.path.join('/', 'Users', username, 'Desktop', volume_name)
            elif vc.os_name == 'Linux':
                volume_path = os.path.join('/', 'home', username, 'Desktop', volume_name)
            else:
            ...
            sleep(5)
            result = vc.command(['--text', '--non-interactive', '--mount', volume_path, '--password', 'SecretPassword', '--stdin', '--force'])
            print(result)
            sleep(5)
            vc.dismount_volume(volume_path)
